...
import { liveStreamsApi } from '../lib/firebaseApi';
...
const LiveStreams: React.FC<LiveStreamsProps> = ({ user }) => {
  ...
  useEffect(() => {
    const loadStreams = async () => {
      try {
        const data = await liveStreamsApi.getAllLiveStreams();
        setStreams(data as LiveStreamType[]);
      } catch (error) {
        console.error('Error loading streams from Firestore:', error);
        setStreams([]);
      }
    };
    loadStreams();
  ...
  const handleCreateStream = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save to Firestore
      const { id: _localId, ...streamPayload } = newStream;
      const created = await liveStreamsApi.createLiveStream(streamPayload);
      setStreams([created as LiveStreamType, ...streams]);
  ...
  const handleStartScheduledStream = async (streamId: string) => {
    try {
      await liveStreamsApi.startStream(streamId);
  ...
  const handleDeleteStream = async (streamId: string) => {
    if (confirm('Are you sure you want to delete this stream?')) {
      try {
        await liveStreamsApi.deleteLiveStream(streamId);
  ...
  const handleLikeStream = async (streamId: string) => {
    try {
      await liveStreamsApi.likeStream(streamId, 1);
      setStreams(streams.map(s => 
  ...
  const handleEndStream = async () => {
    if (selectedStream) {
      try {
        await liveStreamsApi.endStream(selectedStream.id);
        setStreams(streams.map(s => 
