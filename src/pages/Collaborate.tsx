...
import { collaborationsApi } from '../lib/firebaseApi';
...
const Collaborate: React.FC<CollaborateProps> = ({ user }) => {
  ...
  useEffect(() => {
    const loadCollaborations = async () => {
      try {
        const data = await collaborationsApi.getAllCollaborations();
        setCollaborations(data as Collaboration[]);
      } catch (error) {
        console.error('Error loading collaborations from Firestore:', error);
        setCollaborations([]);
      }
    };
    loadCollaborations();
  ...
  const handleApply = async (collabId: string) => {
    ...
    try {
      await collaborationsApi.applyToCollaboration(collabId, applicant);
  ...
  const handleCreateCollaboration = async () => {
    ...
    try {
      const { id: _localId, ...collabPayload } = newCollab;
      const created = await collaborationsApi.createCollaboration(collabPayload);
      setCollaborations([created as Collaboration, ...collaborations]);
