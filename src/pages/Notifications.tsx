...
import { notificationsApi as firebaseNotificationsApi } from '../lib/firebaseApi';
...
const Notifications: React.FC<NotificationsProps> = ({ user }) => {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      try {
        const apiNotifications = await firebaseNotificationsApi.getNotifications(userId);
  ...
  const markAllAsRead = async () => {
    try {
      await firebaseNotificationsApi.markAllAsRead(userId);
  ...
  const markAsRead = async (id: string) => {
    try {
      await firebaseNotificationsApi.markAsRead(id);
      setNotifications(prev =>
        prev.map(notif =>
  ...
  const deleteNotification = async (id: string) => {
    try {
      await firebaseNotificationsApi.deleteNotification(id);
