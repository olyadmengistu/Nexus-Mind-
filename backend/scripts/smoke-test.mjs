...
async function getAuthToken(email, password) {
  ...
  if (data.error && data.error.message === 'EMAIL_EXISTS') {
    ...
    if (!response.ok) {
      throw new Error(`Failed to sign in existing user: ${response.status} ${JSON.stringify(signInData)}`);
    }
    return signInData.idToken;
  }

  throw new Error(`Failed to create auth user: ${response.status} ${JSON.stringify(data)}`);
}

async function apiRequest(pathUrl, options = {}) {
  const url = `${BACKEND_BASE}${pathUrl}`;
  const response = await fetch(url, options);
  const text = await response.text();
  let body;
  try { body = text ? JSON.parse(text) : null; } catch { body = text; }
  if (!response.ok) {
    throw new Error(`API ${pathUrl} failed ${response.status}: ${JSON.stringify(body)}`);
  }
  return body;
}

async function authRequest(pathUrl, token, options = {}) {
  const init = {
    ...options,
    headers: {
  ...
  return apiRequest(pathUrl, init);
}

async function run() {
  console.log('Using backend base:', BACKEND_BASE);
  console.log('Signing in with Firebase email/password account...');
  const idToken = await getAuthToken('smoke-test-user@nexusmind.local', 'SmokeTest123!');
